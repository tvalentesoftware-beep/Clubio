...
python db/correr.py db/migraciones/0001_esquema_inicial.sql db/migraciones/0002_disponibilidad.sql
python db/correr.py db/semillas/0001_clubes_demo.sql
python db/correr.py db/pruebas/0001_reglas_del_modelo.sql db/pruebas/0002_disponibilidad.sql
```

Si el esquema está bien, la salida de las pruebas es una lista de `OK`. Si
algo se rompió, corta con un `FALLO` que dice cuál.

Estado al 2026-09-15: las dos migraciones y la semilla entran limpias en
Supabase (PostgreSQL 17) y las 21 pruebas pasan.
